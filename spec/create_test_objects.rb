...
module TestData
  ...
  def self.create_invoice_items
    ...
    Invoice.all.each do |invoice|
      k = 0 
      3.times do 
        invoice.invoice_items.create!(quantity: invoice_item_data[j][0], unit_price: ( i + k >= 19) ? items[i - k].unit_price : items[i + k].unit_price, status: invoice_item_data[j][2], item_id:( i + k >= 19) ? items[i - k].id : items[i + k].id)
        j += 1
        k += 1
      end
      i += 1
  ...
  def self.create_transactions
    i = 0
    Invoice.all.each do |invoice|
      invoice.transactions.create!(credit_card_number: transaction_data[i][0].delete('-')[0..15],result: transaction_data[i][1] )
      i += 1
    end
