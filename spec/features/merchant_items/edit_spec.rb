...
RSpec.describe 'Merchant Items Show Page' do
  ...
  describe "edit a merchant's item" do
    ...
    it "can update a merchant's item" do
      ...
      fill_in('Description',
        with: "Mad and hairy? You wouldn't be taking about me, now, would you?")
      ...
      expect(page).to have_content(
        "Mad and hairy? You wouldn't be taking about me, now, would you?")
      expect(page).to have_content('$947.53')
      expect(page).to have_content(
        "#{@item1.reload.name}'s information was successfully updated!")
      expect(page).to_not have_content(
        "#{@item1.reload.name}'s information was not successfully updated!")
    end
  end
end
